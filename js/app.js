...
const Storage = {
    getPosts() {
        const posts = localStorage.getItem('inspiration_notes');
        return posts ? JSON.parse(posts) : [];
    },
    
    savePosts(posts) {
        localStorage.setItem('inspiration_notes', JSON.stringify(posts));
    },
...
const Renderer = {
    renderPosts() {
        ...
        // 更新笔记数量
        if (countEl) {
            countEl.textContent = `${posts.length} 条笔记`;
        }
        
        if (posts.length === 0) {
            container.innerHTML = '<div class="empty-message">还没有笔记，快来记录第一条灵感吧！</div>';
            return;
        }
...
async function publishPost() {
    ...
    imageInput.value = '';
    document.querySelector('.file-name').textContent = '📎 添加图片';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('image').addEventListener('change', function() {
        const fileName = this.files[0] ? this.files[0].name : '📎 添加图片';
